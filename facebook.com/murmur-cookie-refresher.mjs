 ...
 *
 * Usage:
 *   node murmur-cookie-refresher.mjs [--email <your-email>] [--hf-token-file path]
 *
 * Env:
 *   MURMUR_REFRESH_HF_TOKEN   - HF token for the murmur Space (or --hf-token-file)
 *   MAINFRAME_BROWSERUI_EMAIL - target browserui profile email
...
const DEFAULTS = {
  ...
  hfTokenFile:
    process.env.MURMUR_REFRESH_HF_TOKEN_FILE ||
    join(
      homedir(),
      "AppData",
      "Roaming",
...
function parseArgs(argv) {
  ...
  for (let i = 0; i < argv.length; i++) {
    const t = argv[i];
    if (t === "--email" || t === "-e") args.email = argv[++i];
    else if (t.startsWith("--email=")) args.email = t.slice("--email=".length);
    else if (t === "--hf-token-file") args.hfTokenFile = argv[++i];
    else if (t.startsWith("--hf-token-file=")) args.hfTokenFile = t.slice("--hf-token-file=".length);
    else if (t === "--murmur-url") args.murmurUrl = argv[++i];
    else if (t.startsWith("--murmur-url=")) args.murmurUrl = t.slice("--murmur-url=".length);
    else if (t === "--port" || t === "-p") args.port = Number(argv[++i]);
    else if (t.startsWith("--port=")) args.port = Number(t.slice("--port=".length));
    else if (t === "--help" || t === "-h") {
      console.log("Usage: node murmur-cookie-refresher.mjs [options]");
      console.log("Options:");
      console.log(`  --email <email>         BrowserUI profile email (default: ${DEFAULTS.email})`);
      console.log(`  --hf-token-file <path>  HF token file path`);
      console.log(`  --murmur-url <url>      Murmur HF Space URL`);
      console.log(`  --port <n>              Remote debugging port`);
      process.exit(0);
    }
  }
  return args;
...
async function clickContinueAs(ws) {
  // Look for "Continue as" button text in various forms Facebook uses
  const script = `
    (function() {
...
async function main() {
  ...
  let hfToken = "";
  try {
    hfToken = readFileSync(args.hfTokenFile, "utf-8").trim();
  } catch {
    throw new Error(`HF token file not readable: ${args.hfTokenFile}`);
  }
  if (!hfToken) {
  ...
  // Always headed - we need to see and interact with the page
  const browserArgs = [
  ...
  try {
    ...
    // Check for "Continue as" button and click it
    ...
    // Extract cookies after potential click
