...
- Recurring YouTube channel checks: mainframe GWS profile (email from the mainframe gws helper) + this folder's watcher: `youtube.com-stream-watch-codex-automation.mjs` (config: `youtube.com-stream-watch-codex-automation.config.json` — gitignored, holds the notify thread id; see `.example.json`).
...
- Notifier: `tools\codex_notify.mjs` (this workspace) → Murmur's queued `/api/automation/notifications` endpoint for the messenger thread configured in `youtube.com-stream-watch-codex-automation.config.json` (gitignored; the thread id is personal — never commit it).
