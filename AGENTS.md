...
- **after any heal/re-login, sync the fresh official state into the mainframe profile immediately** (`Copy-Item "$env:SCOOP\persist\bitwarden-cli\bw-data\data.json" "$env:APPDATA\mainframe\accounts\bitwarden\<email>\data.json" -Force`, keep the stale one as `<data.json.bak-stalekeys-<ts>>`); otherwise the helper's Install-ProfileData restores the OLD-password state on the next `run` and the key mismatch comes back. done 2026-08-13 after the first heal.
