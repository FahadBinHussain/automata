...
- ~~merged music tracker~~ (0.6.0+): the spotify/soundcloud visited-track userscript (old `spotify.com/spotify-track-history-tracker-and-smart-youtube-search-integration`) is now the same file, gated by `IS_YT` / `IS_MUSIC`; both sides share one neon conn string (settings panel is on the youtube side, `connStr()` is shared) and one `neonQuery`. the old google apps script backend is DEAD (`script.googleusercontent.com` echo returns 404, cold start ~43s > 15s timeout) - visited history now lives in neon `visited_tracks (id text pk, name, source check('spotify','soundcloud'), updated_at)`. backfilled 1198 spotify + 4 soundcloud rows from the google sheet "Spotify History" (`1xWa2uDiUAMwQLTtOXAEb531rpmCOPyhTOSKJnTsdZOE`, tabs Spotify/SoundCloud, cols A=id B="artist - song" C=US-locale timestamp) using `on conflict (id) do nothing` + separate UPDATE pass; sheet kept untouched, nothing deleted.
- music module details: `vtrackId` normalizes track ids (leading slashes stripped both on save and fetch); `mFetchHistory()` replaces the local set from the cloud per source; `mSave()` inserts with `on conflict (id) do update set name = excluded.name, updated_at = now()` (upsert) or deletes; offline banner + retry with exponential backoff when conn string missing/fetch fails. GM storage keys for the music side are `mVisitedSet`, `mVisitedCache`, `mVisitedLog` (must never collide with youtube-side keys).
